    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', minHeight: '1122px', background: 'white', fontSize: `${fontSize}px` }}>
      ...
      <div style={{ padding: '60px', borderRight: '1px solid #e2e8f0' }}>
        ...
        <section style={{ marginBottom: `${sectionSpacing}px` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '15px' }}>
            ...
            <h3 style={{ fontSize: '16px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1px' }}>{labels.experience[lang]}</h3>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
            {data.experience.map((exp) => (
              <div key={exp.id}>
                ...
                <BulletList text={exp.description[lang]} fontSize={13} lineHeight={1.6} bulletColor={accentColor} style={{ color: '#475569' }} />
              </div>
            ))}
          </div>
        </section>
        ...
        <section>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '15px' }}>
            ...
            <h3 style={{ fontSize: '16px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1px' }}>{labels.education[lang]}</h3>
          </div>
      ...
      <div style={{ background: '#f8fafc', padding: '60px 30px' }}>
        <section style={{ marginBottom: '40px' }}>
          <h3 style={{ fontSize: '14px', fontWeight: 800, textTransform: 'uppercase', marginBottom: '20px', color: '#0f172a' }}>{labels.contact[lang]}</h3>
        ...
        <section style={{ marginBottom: '40px' }}>
          <h3 style={{ fontSize: '14px', fontWeight: 800, textTransform: 'uppercase', marginBottom: '20px', color: '#0f172a' }}>{labels.profile[lang]}</h3>
          <p style={{ fontSize: '12px', lineHeight: '1.6', color: '#475569' }}>{data.summary[lang]}</p>
        </section>

        <section>
          <h3 style={{ fontSize: '14px', fontWeight: 800, textTransform: 'uppercase', marginBottom: '20px', color: '#0f172a' }}>{labels.skills[lang]}</h3>
